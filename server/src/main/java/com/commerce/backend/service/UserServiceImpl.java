...
public class UserServiceImpl implements UserService {
    ...
    public UserServiceImpl(UserRepository userRepository,
                           ...
                           CartRepository cartRepository, OrderRepository orderRepository, UserResponseConverter userResponseConverter) {
    ...
    public boolean checkAdmin() {
        ...
        return "Admin".equalsIgnoreCase(user.getPermissions());
    }

    @Override
    public List<UserResponse> getUsers() {
        return userRepository.findAll().stream().map(user->userResponseConverter.apply(user)).collect(Collectors.toList());
    }
    ...
    public void deleteUser(String email) {
        ...
        if(userOp.isPresent()){
            ...
            orderRepository.deleteByUser_Id(user.getId());
            userRepository.delete(user);
        }else throw new ResourceNotFoundException("Email ne trouves pas");
    }

    @Override
    public void activate(Boolean emailVerified, String email) {
        Optional<User> userOp=userRepository.findByEmail(email);
        if(userOp.isPresent()){
            User user=userOp.get();
            if(emailVerified!=null){
                user.setEmailVerified(emailVerified.booleanValue()?1:0);
