...
export class ResetDetailsComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.detailsForm = new FormGroup({
      ...
      phone: new FormControl(null, [BlankValidators.checkIfBlankValidator, Validators.pattern('[0-9]+'), Validators.minLength(10), Validators.maxLength(12)]),
    });
  ...
  onSubmitDetailsForm() {
    ...
    this.accountService.updateUser(user)
      .pipe(take(1), catchError(
        error => {
          alert('Une erreur s;est produite. Veuillez actualiser votre page.');
          return throwError(error);
        }
      ))
      .subscribe(data => {
        this.innerLoading = false;
        alert('Succès! Vos informations ont été modifiées.');
      });
